 ...
 *      }
 */
import { _decorator, Button, Component, Node, ScrollView, Enum, CCInteger, Vec2, v2, Size, EventHandler, UITransform, v3 } from 'cc';
...
export default class TableView extends Component {
    ...
    @property({type: EventHandler, tooltip: "单元格的数量\n函数形参 () => number"})
    private handlerCellCount: EventHandler = new EventHandler();

    @property({type: Component.EventHandler, tooltip: "单元格的大小\n函数形参 (idx: number) => cc.Size"})
    ...
    private tableCellSize: (idx: number) => Size = null;                              //索引的单元格大小获取函数
    ...
    public reloadData(isUseLastOffset: boolean = false) {
        ...
        if (this.scrollView.horizontal === this.scrollView.vertical) {
            ...
            console.log("TableView only vertical or horizontal scrolling is supported")
            return;
        }
    ...
    private _updateContentSize(isUseLastOffset: boolean) {
        let size = this.maskNode.getComponent(UITransform).contentSize;
        let cellsCount = this.tableCellCount();
        if (cellsCount > 0) {
            let maxPosition = this._cellsPositions[cellsCount];
            if (this.scrollView.horizontal && maxPosition > this.maskNode.getComponent(UITransform).width)
                size = new Size(maxPosition, this.maskNode.getComponent(UITransform).height);
            else if (this.scrollView.vertical && maxPosition > this.maskNode.getComponent(UITransform).height)
                size = new Size(this.maskNode.getComponent(UITransform).width, maxPosition);
        }
        if (size.width != this.scrollView.content.getComponent(UITransform).width || size.height != this.scrollView.content.getComponent(UITransform).height) {
    ...
    //索引转坐标
    private _positionFromIndex(index): Vec2 {
        ...
        if (this.scrollView.vertical) {
            if (this.vordering === VerticalFillOrder.BOTTOM_UP)
                ...
                pos.y = this.scrollView.content.getComponent(UITransform).height - pos.y - size.height / 2;
        } else
            pos.x = pos.x + size.width / 2;
        if (this.scrollView.vertical) {
            pos.x = this.scrollView.content.getComponent(UITransform).width * (0.5 - this.scrollView.content.getComponent(UITransform).anchorX);
            pos.y = pos.y - this.scrollView.content.getComponent(UITransform).height * this.scrollView.content.getComponent(UITransform).anchorY;
        } else {
            pos.x = pos.x - this.scrollView.content.getComponent(UITransform).width * this.scrollView.content.getComponent(UITransform).anchorX;
            pos.y = this.scrollView.content.getComponent(UITransform).height * (0.5 - this.scrollView.content.getComponent(UITransform).anchorY);
        }
        return pos;
    }
    ...
    private _setIndexForCell(index: number, cell: TableViewCellNode) {
        cell.getComponent(UITransform).setAnchorPoint(0.5, 0.5);
        var pos2D = this._positionFromIndex(index)

        cell.setPosition(pos2D.x, pos2D.y, 0);
        cell.setIdx(index);
    }
    ...
    private _scrollViewDidScroll() {
        ...
        if (this.vordering === VerticalFillOrder.BOTTOM_UP)
            offset.y = this.scrollView.content.getComponent(UITransform).height - offset.y - this.maskNode.getComponent(UITransform).height;
        startIdx = this._indexFromOffset(offset.clone());
        if (startIdx === CC_INVALID_INDEX)
            startIdx = countOfItems - 1;

        offset.x += this.maskNode.getComponent(UITransform).width;
        offset.y += this.maskNode.getComponent(UITransform).height;
    ...
    private _createCellAtIndex(idx): TableViewCellNode {
        ...
        cell.getComponent(UITransform).setContentSize(this.tableCellSize(idx));
